        ...
        {recipe.tags && recipe.tags.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mb-3">
            {recipe.tags.slice(0, 3).map((t) => (
              <span
                key={t.id}
                className="text-[11px] font-medium uppercase tracking-wider text-gray-500 bg-gray-100 px-2.5 py-0.5 rounded-full"
              >
                {TAG_EMOJIS[t.name] ? `${TAG_EMOJIS[t.name]} ` : ""}
                {t.name}
              </span>
            ))}
          </div>
          ...
          {recipe.tags && recipe.tags.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-4">
              {recipe.tags.map((t) => (
                <span
                  key={t.id}
                  className="text-[11px] font-medium uppercase tracking-wider text-gray-500 bg-gray-100 px-3 py-1 rounded-full"
                >
                  {TAG_EMOJIS[t.name] ? `${TAG_EMOJIS[t.name]} ` : ""}
                  {t.name}
                </span>
          ...
          {loadingIng ? (
            <div className="space-y-2">
              {[1, 2, 3, 4].map((i) => (
                <div
                  key={i}
                  className="h-9 bg-gray-50 rounded-lg animate-pulse"
                />
              ))}
            </div>
