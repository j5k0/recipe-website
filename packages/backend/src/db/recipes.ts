...
export async function deleteRecipe(recipeId: string): Promise<void> {
  ...
  // Delete in order: ingredients, recipe_tags, then recipe
...
export async function updateRecipe(recipeId: string, input: {
  ...
  // Check existing recipe so we can delete an old image when it gets replaced.
  const { rows: existingRows } = await pool.query<Recipe>(
    'SELECT image FROM recipes WHERE id = $1',
    [recipeId],
  );
  const existingRecipe = existingRows[0];

  if (input.image !== undefined && existingRecipe?.image && input.image !== existingRecipe.image) {
    try {
